...
bot.hears("Замовити столик 🪑", (ctx) => {
  ctx.reply(
    "Ви можете замовити столик онлайн або зателефонувати нашому менеджеру"
  );
  bot.telegram.sendContact(ctx.chat.id, "+380637444337", "Yellow Mellow");
  console.log(this);
});
...
bot.hears("Місцезнаходження🎯", (ctx) => {
  bot.telegram.sendLocation(
    ctx.chat.id,
    (latitude = "49.83870331057144"),
    (longitude = "24.027840313759"),
...
bot.hears("Коктейлі 🍹", (ctx) => {
  ...
  const secondPhoto = () => {
    return ctx.replyWithPhoto({
      source: fs.createReadStream("img/cocktails2.jpg"),
    });
  };
